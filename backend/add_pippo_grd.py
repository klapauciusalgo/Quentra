...
V9_TRADES_PATH = "/home/ubuntu/new-btc-analysis/data/v9_trades_all4_full.csv"

def build_pippo_grd():
    df_raw = pd.read_csv(V9_TRADES_PATH)
    df = df_raw[df_raw["variant"] == "B  GRID 1x"].copy().reset_index(drop=True)
    ...
    # Compounded yearly return figures from verified v11_yoy_returns.csv (GRID 1x):
    compounded_yearly = {
        2020: 191.10,
        2021: 53.37,
        2022: -16.41,
    ...
    # Active Trade #725 (Entered 2026-09-20 19:30:00 @ $81,177.32)
    ...
    strategy_obj = {
        ...
        "risk_tier": "Moderate",
        "recommended_for": "Traders seeking high cumulative returns (+1719.7%, CAGR 54.0%) with active market participation (724 trades) through 30M MA compression and 1H proximity regimes without 4H macro filters.",
        "badge": "+1719.7% Return (CAGR 54.0%)",
        "metrics": {
            "total_return_pct": 1719.70,
            "cagr_pct": 54.00,
            "win_rate_pct": 32.87,
            "profit_factor": 1.43,
            "max_drawdown_pct": -21.44,
            "calmar_ratio": 2.52,
            "total_trades": 724,
            "win_trades": 238,
            "loss_trades": 486,
        ...
        "logic_summary": "High-frequency MA compression sniper with pure 1H regime. Validates 1H price compression within 1.5% of MA25/50. Triggers entry on 30M candle close when MA25 and MA50 are tightly squeezed (<0.15% spread) and ATR < 1.0%, with price within 0.8% of both MAs. Exits defensively when price trades 0.5% below both MAs, protected by a strict -2.0% stop-loss and +20.0% take-profit target with 1.0x position sizing.",
...
def update_strategies_file(file_path, new_strat):
    ...
    # Check if already present, update or insert
    found = False
    ...
    if not found:
        # Insert after pippo-30m-new-gen or at position 2
        insert_pos = 2
        for idx, s in enumerate(strat_list):
            if s.get("id") == "pippo-30m-new-gen":
                insert_pos = idx + 1
                break
        strat_list.insert(insert_pos, new_strat)
